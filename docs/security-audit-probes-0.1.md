...
| PROBE-09 | medium | oversized, special, symlinked, incomplete, or cross-run channel artifacts could confuse the supervisor or filesystem boundary; paths are engine-derived beneath validated stack/replica identities, envelopes are regular files capped at 4 KiB, incomplete worker environments fail closed, and unsafe cleanup entries abort before deletion |
