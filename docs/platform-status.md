...
| Service-aware supervisor | Standalone replica lifecycle, restart reconciliation, guest-signalled application health, stale-supervisor detection, readiness waits, whole-generation rollback, deterministic rolling decisions, durable rollout intent, and isolated old/candidate ownership foundations work; daemon-driven concurrent action execution, provider-first activation, graceful draining, typed service wiring, HTTP/command probes, ready-only routing, idle suspension, cancellation trees, circuit breaking, and backpressure remain |
...
The rootless local engine now owns desired-state reconciliation through an authenticated API, retains its complete supervisor/worker process tree across supported operating systems, gates opted-in workloads on application readiness and liveness, and can deterministically plan and durably checkpoint availability-bounded per-replica rolling actions. Concurrent generations now have separate authority and state foundations. The next milestone is daemon execution of those actions with ready-only routing and draining, followed by native authority sandboxes and kernel resource policy.
