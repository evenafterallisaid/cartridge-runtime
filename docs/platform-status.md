...
| Service-aware supervisor | Standalone replica lifecycle, restart reconciliation, guest-signalled application health, stale-supervisor detection, readiness waits, and whole-generation rollback work; provider-first activation, typed service wiring, HTTP/command probes, surge/unavailable rolling replacement, idle suspension, cancellation trees, circuit breaking, and backpressure remain |
...
| Complete stack schema | Instances, exact packages, resources, secret slots, restart intent, security policy, and guest-signalled health policy work; services, ingress, jobs, external probe kinds, rollout policy, and runtime secret delivery remain |
