...
The rootless local engine owns desired-state reconciliation through an authenticated API, retains its complete supervisor/worker process tree across supported operating systems, gates opted-in workloads on application readiness and liveness, and executes crash-recoverable availability-bounded per-replica rolling actions across separately authorized generations. The next engine gates are ready-only ingress, state migration policy, native authority sandboxes, kernel resource policy, and OS peer credentials.
