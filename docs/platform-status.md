...
| Service-aware supervisor | Standalone replica lifecycle, restart reconciliation, guest-signalled application health, stale-supervisor detection, readiness waits, whole-generation rollback, and the bounded deterministic rolling decision engine work; durable concurrent-generation execution, provider-first activation, typed service wiring, HTTP/command probes, ready-only routing, idle suspension, cancellation trees, circuit breaking, and backpressure remain |
