...
Updates can cross a durable rollout transaction instead of mutating desired state directly. Preparation stores a checksum-bound candidate plus the exact previous revision, generation, state, and plan without activating it. Activation re-verifies installed package bytes and appends the candidate to the immutable desired-state journal. Commit requires a ready health report; running replicas must also remain continuously healthy through a server-enforced two-second stability window, measured from application readiness when a probe is configured, while successfully completed jobs can commit immediately. The health digest is retained in the checkpoint. Cancellation changes no desired state; rollback appends the exact previous plan or tombstone after re-verifying every retained package. Active transactions fence ordinary apply, stop, and remove operations. If the process dies after a journal append but before replacing its checkpoint, the next rollout read derives and persists the only valid next phase from the adjacent journal generations. Terminal checkpoints are archived under their digest before a later rollout replaces the current record.

A separate deterministic scheduler validates generation-separated replica observations and selects starts, drains, waits, completion, or rollback under plan-bound surge, unavailable, minimum-ready, progress-deadline, and drain-timeout policy. Activated rollouts persist a bounded checksummed execution checkpoint with independent old/new topology, per-ordinal start and drain intent, monotonic sequencing, and crash recovery. Old and candidate targets receive separate supervisor locks, runtime-status files, probe roots, and generation-keyed mutable state. Exact rollout identity gates that temporary dual authority. The daemon executes those actions through concurrent generation supervisors, waits for configured application readiness, shares one absolute deadline across graceful batch drains, acknowledges completed drains durably, and recovers or rolls back after restart. Aggregate active capacity is checked across the whole topology, not only matching instance names. Ready-only local invocation now pins bounded requests to exact runs; network ingress, state migration gates, and canaries remain later layers.
