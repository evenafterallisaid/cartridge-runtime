...
A separate deterministic scheduler validates generation-separated replica observations and selects starts, drains, waits, completion, or rollback under plan-bound surge, unavailable, minimum-ready, progress-deadline, and drain-timeout policy. Activated rollouts can persist a bounded checksummed execution checkpoint with independent old/new topology, per-ordinal start and drain intent, monotonic sequencing, and crash recovery. Old and candidate targets receive separate supervisor locks, runtime-status files, probe roots, and generation-keyed mutable state. Exact rollout identity gates that temporary dual authority. The live daemon still replaces one whole generation at a time; action execution, graceful drain completion, ready-only traffic routing, and canaries remain the next layers.
