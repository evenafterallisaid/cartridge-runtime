...
Each active supervisor refreshes the checksum-bound observed generation on a fixed heartbeat. The engine derives a bounded health report from desired state, generation identity, replica phases, and heartbeat age; reports are hashed, strictly validated, inventory-capped, and returned only through the authenticated daemon channel. Readiness means every desired replica is running or completed successfully. It deliberately does not infer application health from process existence. The client-side wait operation has its own deadline, narrows every daemon request to the remaining budget, and fails early when a generation reaches an unrecoverable failed or degraded state.

Updates can cross a durable rollout transaction instead of mutating desired state directly. Preparation stores a checksum-bound candidate plus the exact previous revision, generation, state, and plan without activating it. Activation re-verifies installed package bytes and appends the candidate to the immutable desired-state journal. Commit requires a ready health report; running replicas must also remain continuously live through a server-enforced two-second stability window, while successfully completed jobs can commit immediately. The health digest is retained in the checkpoint. Cancellation changes no desired state; rollback appends the exact previous plan or tombstone after re-verifying every retained package. Active transactions fence ordinary apply, stop, and remove operations. If the process dies after a journal append but before its checkpoint replacement, the next rollout read derives and persists the only valid next phase from the adjacent journal generations. Terminal checkpoints are archived under their digest before a later rollout replaces the current record. This provides crash-consistent recovery and automatic failed-update rollback, but it still replaces one whole generation at a time; surge/unavailable windows, canaries, and application probes are separate scheduler layers.
