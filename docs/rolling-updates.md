...
The local invocation broker routes only to ready runs and cancels work when its pinned target leaves membership. There is still no network-facing HTTP listener or connection-draining proxy, so this is not yet a claim of zero-downtime HTTP traffic. Stateful generation handoff also waits for the migration/rollback-receipt gate; generation-keyed state deliberately prevents implicit sharing during a rollout.
